...
const ReservationCalendar = () => {
      ...
      try {
        ...
        if (response.status === 1 && Array.isArray(response.result)) {
          ...
          if (Array.isArray(response.totalVisitors)) {
            const processedBookings = response.totalVisitors.flatMap(
              (visitor) => {
                ...
                while (currentDate <= checkOut) {
                  dates.push({
                    ...
                    paymentStatus:
                      visitor.paymentStatus || PAYMENT_STATUS.UNPAID,
  ...
  const toggleReservation = (roomId, date) => {
    const exists = selectedReservations.some(
      (res) =>
        res.roomId === roomId && res.date.toDateString() === date.toDateString()
    );
  ...
  const getBookingInfo = (roomId, date) => {
    return bookings.find(
      (b) =>
        b.roomId === roomId && b.date.toDateString() === date.toDateString()
    );
  };
  ...
  const handleMouseEnter = (roomId, date) => {
    ...
    const newReservations = selectedReservations.filter(
      (res) =>
        res.roomId !== roomId || res.date < startDate || res.date > endDate
    );
          ...
          {rooms.map((room) => (
            ...
            >
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                const isPast =
                  thisDate < new Date(new Date().setHours(0, 0, 0, 0));
